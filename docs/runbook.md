...
## 3. Turn on email-PIN auth (Elastic Email)
...
| `FROM_EMAIL` | A verified sender address in your Elastic Email account |

Click **Deploy** at the top of the Variables section to push them
live. No code change needed.

### First sign in
1. Visit `/admin/users` while still in open mode — your email
   `ross@thinkrapid.com` is already in the users table.
2. Add any other users you want to allow.
3. Set the three secrets above.
4. Visit `/login`, enter your email. Elastic Email delivers the PIN.
5. Enter the PIN. You're in. The session cookie is good for 30 days.

If `ELASTIC_EMAIL_API_KEY` is unset, login still works in a debug
mode — the PIN is logged to the Worker console (visible in
**Workers & Pages → flower-hq → Logs**) instead of being emailed.
