...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  if (intent === "parse") {
    ...
    if (!(file instanceof File) || file.size === 0) {
      return { error: "Select a .iif file" };
    }
    const content = await file.text();
    const entries = parseIif(content);
    if (entries.length === 0) {
      return { error: "No journal entries found in this file" };
    }
  ...
  if (intent === "import") {
    const fileContent = String(form.get("fileContent") || "");
    const filename = String(form.get("filename") || "unknown.iif");
    if (!fileContent) return { error: "Missing file content" };

    const entries = parseIif(fileContent);
    if (entries.length === 0) return { error: "No entries to import" };
    ...
    for (const je of entries) {
      ...
      await db.insert(qboImportLog).values({
        ...
        totalDebits: v.totalDebits,
        status,
        qboJeId: result.qboId ?? null,
        errorDetail:
          result.error === "already_exists" ? null : (result.error ?? null),
        importedBy: user.id,
      });
    }
    ...
    <div className="space-y-6">
      {!connected && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
          Not connected to QBO. Go to{" "}
          <a href="/qbo/connect" className="underline">
            Connection
          </a>{" "}
          first.
        </div>
      ...
      {actionData?.parsed && (
        <section className="space-y-4">
          <PageHeader
            ...
            subtitle={`${actionData.parsed.entries.length} journal ${actionData.parsed.entries.length === 1 ? "entry" : "entries"}`}
          />
          ...
          <div className="flex gap-3">
            <Form method="post">
              ...
              <input
                type="hidden"
                name="filename"
                value={actionData.parsed.filename}
              />
              <input
                type="hidden"
                name="fileContent"
              ...
              <p className="mt-1 text-xs text-neutral-500">
                Note: re-upload the file and use the browser's import
                form below for the actual POST. This preview confirms
                parsing is correct.
              </p>
            </Form>
            <a href="/qbo" className="text-sm text-neutral-600 hover:underline self-center">
              Cancel
            </a>
          </div>

          {/* Combined parse + import form */}
          <div className="rounded-lg border border-neutral-200 bg-white p-5">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-neutral-500">
              Preview looks good? Import it
            </h2>
            <Form
              method="post"
              encType="multipart/form-data"
              ...
              onSubmit={async (e) => {
                ...
                if (!file) return;
                const content = await file.text();
                const hiddenContent = formEl.querySelector(
                  'input[name="fileContent"]',
                ) as HTMLInputElement;
                hiddenContent.value = content;
                const hiddenFilename = formEl.querySelector(
                  'input[name="filename"]',
                ) as HTMLInputElement;
            ...
            >
              ...
              <Field label="Re-select the same .iif file">
                <Input name="file" type="file" accept=".iif" required />
              </Field>
              <Button type="submit" disabled={!connected}>
